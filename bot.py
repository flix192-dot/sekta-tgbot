...
async def gemini_classify_intent(text: str, player: str) -> dict:
    ...
    """
    if not GEMINI_API_KEY:
        parsed = parse_event(text)  # фолбэк на regex
    ...
    system = (
        ...
        'Если это обычный вопрос, реплика, разговор о дисциплине, мотивации, жизни '
        'или что угодно, не являющееся прямой просьбой добавить что-то в календарь — '
        'это НЕ событие. Не пытайся притянуть обычное сообщение к событию. '
