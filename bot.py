...
def today_kyiv_str() -> str:
    """Дата отчёта всегда по Киеву, формат dd.mm.yyyy — как в S.reviews на сайте."""
    return datetime.now(TZ_ARTEM).strftime('%d.%m.%Y')

def upsert_review_habit(player: str, habit_id: str, done: bool = True, photo_data_url: str | None = None) -> bool:
    """
    Обновляет (или создаёт) сегодняшний отчёт в /sekta/reviews — именно то,
    что показывается партнёру в разделе «Отчёты» на сайте, и именно там сайт
    ищет прикреплённые фото (ключ '{habitId}_{dayIdx}' в поле photos).
    Вызывается из mark_habit_done, чтобы отчёт на сайте обновлялся сразу,
    без необходимости нажимать «Отправить отчёт» вручную.
    """
    try:
        ...
        today_str = today_kyiv_str()

        # Уже одобренный отчёт за сегодня не трогаем
        approved = next(
            (r for r in reviews if isinstance(r, dict) and r.get('from') == player
             and r.get('date') == today_str and r.get('status') == 'approved'),
            None
        )
        if approved:
            return True

        day_idx  = now_for(player).date().weekday()
        ...
        if target is None:
            target = {
                ...
                'from':        player,
                'date':        today_str,
                'dateTs':      int(time.time() * 1000),
                'habits':      {},
                'habitTimes':  {},
                'photos':      {},
                'submittedAt': datetime.now(TZ_ARTEM).isoformat(),
                'status':      'pending',
                'reviewedAt':  None,
                'reviewedBy':  None,
            }
        ...
        target.setdefault('habits', {})
        target.setdefault('habitTimes', {})
        target.setdefault('photos', {})

        target['habits'][habit_id] = done
        if done:
            target['habitTimes'][habit_id] = time_str
        target['submittedAt'] = datetime.now(TZ_ARTEM).isoformat()
        target['status']      = 'pending'

        if photo_data_url:
            photo_key = f'{habit_id}_{day_idx}'
            target['photos'][photo_key] = photo_data_url

        ref.set(reviews)
        logger.info(f"📋 Review upserted for {player}/{habit_id} (photo={'yes' if photo_data_url else 'no'})")
        return True
    except Exception as e:
        logger.error(f"upsert_review_habit error: {e}")
        return False

def mark_habit_done(player: str, habit_id: str, photo_data_url: str | None = None) -> bool:
    """Ставит галочку на привычку в Firebase за сегодня и обновляет отчёт (+ фото, если передано)."""
    try:
        ...
        # Обновляем отчёт (reviews) — именно его партнёр видит на сайте, включая фото
        upsert_review_habit(player, habit_id, True, photo_data_url)
...
# Резервная копия дефолтных книг с сайта (id -> (название, кол-во страниц)) —
# используется только для красивого сообщения в логе, если книга не кастомная.
DEFAULT_BOOKS = {
    'b1':  ('Богатый папа, бедный папа',        336),
    'b2':  ('Думай и богатей',                  320),
    'b3':  ('Самый богатый человек в Вавилоне', 176),
    'b4':  ('Психология денег',                 256),
    'b5':  ('Инвестор-миллионер',               288),
    'b6':  ('Атомные привычки',                 320),
    'b7':  ('48 законов власти',                452),
    'b8':  ('Влияние',                          340),
    'b9':  ('Мышление быстрое и медленное',     499),
    'b10': ('Сила настоящего',                  224),
    'b11': ('Не давай эмоциям управлять тобой', 256),
    'b12': ('5 утренних ритуалов',              288),
    'b13': ('Магия утра',                        208),
}

def get_book_info(book_id: str | None) -> tuple[str, int] | None:
    """Пытается найти название и кол-во страниц книги — сперва в кастомной библиотеке
    игрока на сайте (/sekta/books/library), затем в дефолтном списке."""
    if not book_id:
        return None
    try:
        library = db.reference('/sekta/books/library').get() or []
        if isinstance(library, dict):
            library = list(library.values())
        for b in library:
            if isinstance(b, dict) and b.get('id') == book_id and b.get('title'):
                return b['title'], b.get('pages') or 0
    except Exception as e:
        logger.warning(f"get_book_info library lookup error: {e}")
    return DEFAULT_BOOKS.get(book_id)

def save_reading_page(player: str, page: int) -> tuple[bool, bool]:
    """
    Сохраняет текущую страницу книги ИМЕННО там, где её читает сайт —
    /sekta/books/{player}/pages (виджет «Чтение»), плюс запись в общий лог книг.
    Возвращает (успех, выбрана_ли_книга_на_сайте).
    """
    try:
        book_ref   = db.reference(f'/sekta/books/{player}')
        book_state = book_ref.get() or {}
        if not isinstance(book_state, dict):
            book_state = {}
        book_id    = book_state.get('bookId')
        prev_pages = book_state.get('pages', 0) or 0

        db.reference(f'/sekta/books/{player}/pages').set(page)

        # Запись в общий лог книг (как делает сайт при нажатии «Сохранить»)
        name      = PLAYER_NAMES[player]
        diff      = page - prev_pages
        diff_str  = f' (+{diff})' if diff > 0 else (f' ({diff})' if diff < 0 else '')
        info      = get_book_info(book_id)
        if info:
            title, total_pages = info
            msg = f'«{title}» — {page} / {total_pages} стр.{diff_str}' if total_pages else f'«{title}» — {page} стр.{diff_str}'
        else:
            msg = f'Страница {page}{diff_str}'

        time_str = datetime.now(PLAYER_TZ[player]).strftime('%d.%m %H:%M')
        log_ref  = db.reference('/sekta/books/log')
        log      = log_ref.get() or []
        if isinstance(log, dict):
            log = list(log.values())
        log.insert(0, {'player': name, 'msg': msg, 'time': time_str})
        log_ref.set(log[:60])

        # Также обновляем старые вспомогательные пути (история по дням недели)
        today, week_key = _week_key_for(player)
        day_idx = today.weekday()
        db.reference(f'/sekta/habitNotes/{week_key}/{player}/reading').set({
            'page': page, 'day': day_idx,
            'time': datetime.now(PLAYER_TZ[player]).strftime('%H:%M'),
        })

        logger.info(f"📖 Book page saved for {player}: {page} (book_id={book_id})")
        return True, bool(book_id)
    except Exception as e:
...
async def handle_habit_report(update: Update, uid: int, player: str, text: str) -> bool:
    ...
    if habit_id == 'gratitude':
        # Сначала просим прислать саму благодарность отдельным сообщением
        waiting_for_gratitude[uid] = True
        await update.message.reply_text(
            '🙏 Отправьте вашу благодарность',
            parse_mode='Markdown'
        )
        return True
    ...
    if habit_id == 'reading':
        ...
        if m:
            page = int(m.group(1))
            saved, has_book = save_reading_page(player, page)
            ok = saved and mark_habit_done(player, habit_id)
            if ok:
                extra = '' if has_book else '\n⚠️ На сайте не выбрана книга — выбери её в разделе «Чтение», чтобы видеть прогресс.'
                await update.message.reply_text(
                    f'📖 *Чтение книг* — отмечено!\nСтраница {page} сохранена на сайте.{extra}',
                    parse_mode='Markdown'
                )
            else:
...
async def handle_proof_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Скачиваем фото и превращаем в data URL — именно в таком формате
    # сайт хранит фото в отчётах (S.reviews[...].photos)
    b64 = None
    try:
    ...
    ok = mark_habit_done(player, habit_id, photo_data_url=b64)
    if ok:
        if b64:
            await update.message.reply_text(
                f'✅ *{name}* — принято!\n\nГалочка поставлена, фото прикреплено к отчёту на сайте 📎',
                parse_mode='Markdown'
            )
        else:
            await update.message.reply_text(
                f'✅ *{name}* — галочка поставлена, но фото сохранить не удалось. Отчёт всё равно обновлён.',
                parse_mode='Markdown'
            )
    else:
        await update.message.reply_text('⚠️ Не удалось поставить галочку. Попробуй снова.')
...
async def handle_pending(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in waiting_for_page:
        ...
        page = int(m.group(1))
        saved, has_book = save_reading_page(player, page)
        ok = saved and mark_habit_done(player, 'reading')
        if ok:
            extra = '' if has_book else '\n⚠️ На сайте не выбрана книга — выбери её в разделе «Чтение», чтобы видеть прогресс.'
            await update.message.reply_text(
                f'📖 *Чтение книг* — отмечено!\nСтраница {page} сохранена на сайте.{extra}',
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('habitmark:'):
        ...
        if habit_id == 'gratitude':
            waiting_for_gratitude[uid] = True
            await query.answer()
            await query.message.reply_text('🙏 Отправьте вашу благодарность')
            return
