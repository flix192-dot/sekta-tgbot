...
HABITS_NO_PROOF = {
    ...
    'reading':   ('чтение',       ['почитал', 'прочитал', 'читал', 'книгу', 'страницы']),
}
...
def mark_habit_done(player: str, habit_id: str) -> bool:
    ...
    try:
        ...
        # Ключ недели — находим текущую неделю
        # Сайт хранит данные как weekly[weekKey][player][habitId][dayIdx]
        # weekKey = строка вида "2026-W27"
...
async def handle_habit_report(update: Update, uid: int, player: str, text: str) -> bool:
    ...
    Если нашли → для привычек с фото просим доказательство,
                  для привычек без фото сразу ставим галочку.
    ...
    if habit_id in HABITS_NEED_PROOF:
        ...
        proof_prompts = {
            ...
            'gratitude': '📸 Отправь скрин или фото что записал благодарности',
        ...
        # Привычка без фото — ставим сразу
