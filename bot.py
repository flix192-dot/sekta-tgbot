...
def find_plans_by_query(plans: list[dict], query: str) -> list[dict]:
    """
    ...
    target_date: date | None = resolve_date(query_lower)
...
async def handle_delete_plan(update: Update, uid: int, player: str, text: str) -> bool:
    ...
    # Ищем совпадения
    matches = find_plans_by_query(plans, text)
...
async def process_user_text(update: Update, uid: int, player: str, text: str):
    ...
    if not parsed.get('time'):
        ...
        await update.message.reply_text(f'⏰ В какое время {parsed["date"].strftime("%d.%m")}?\n`в 14` или `в 9:30`', parse_mode='Markdown')
        return
